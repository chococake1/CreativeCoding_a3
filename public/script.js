document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateBackgroundColor(color, opacity = 0.5) {
        const rgbColor = getComputedStyle(document.body).backgroundColor;
        const rgbaColor = rgbColor.replace('rgb', 'rgba').replace(')', `, ${opacity})`);
        document.body.style.backgroundColor = rgbaColor;
    }

    // Convert hue to rgba color with specified opacity
    function hueToRgba(hue, opacity = 0.5) {
    ...
    document.querySelectorAll('.answer').forEach(button => {
        button.addEventListener('click', event => {
            ...
            if (currentQuestion < questions.length) {
                ...
                if (squareProperties.color.hue !== undefined) {
                    const rgbaColor = hueToRgba(squareProperties.color.hue);
